...
if (Other != none)
{
	if (class'TraceHitSpawner'.default.bForceBulletHole||((!bHandToHand) && bInstantHit && bPenetrating))
	{
			...
			if (hole != None)
			{
				...
				else
				{
                    ...
                    && GetFloorMaterial(texGroup)!='Wood' && GetFloorMaterial(texGroup)!='Concrete' && GetFloorMaterial(texGroup)!='Stone')
						{
                        hole.Texture = Texture'HDTPItems.Skins.HDTPFlatFXtex38'; //CyberP: decal for melee hits.
                        hole.DrawScale = 0.004000;
