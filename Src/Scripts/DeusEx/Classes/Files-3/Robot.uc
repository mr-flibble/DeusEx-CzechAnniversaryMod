...
local ScorchMark mark;
    ...
    mark = Spawn(class'DeusEx.ScorchMark', Self,, HitLocation, Rotator(HitNormal));
		...
		{
			mark.DrawScale = CollisionRadius / 110;
			mark.ReattachDecal();
		}
}
...
function TakeDamageBase(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, name damageType, bool bPlayAnim)
{
    ...
    if (Health < 80 && !IsA('MedicalBot') && !IsA('RepairBot') && !IsA('CleanerBot') && !IsA('SpiderBot3'))
    {
                InitGenerator();
				if (sparkGen != None)
				{
	...
	// flame attacks don't really hurt robots much, either
		...
		actualDamage *= 0.35;  // quarter strength //CyberP: increased so plasma rifle. was 0.25
    ...
    if (Enemy.IsA('DeusExPlayer') && DeusExPlayer(Enemy).PerkNamesArray[14]==1)
