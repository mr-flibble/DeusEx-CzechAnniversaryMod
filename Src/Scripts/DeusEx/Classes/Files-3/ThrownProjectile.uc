...
{
	// CyberP: no lam/grenade climbing exploit
	if (standingActor != None && standingActor.Velocity.Z < -100 && bProximityTriggered)
		{
        bDoExplode = True;
		bArmed = False;
		}
...
simulated function Tick(float deltaTime)
{
    ...
    if (player != none && Owner.IsA('DeusExPlayer'))
    {
    itmp = Player.SkillSystem.GetSkillLevel(class'SkillDemolition');
			...
			blastRadius=1152.000000;
			else
			blastRadius=320.000000;

			if ((DeusExPlayer(Owner) != None) && (DeusExPlayer(Owner).PerkNamesArray[26]==1) && (IsA('GasGrenade')))
		    Damage=260.000000;
    }
	...
	if ( Role == ROLE_Authority )
	{
		...
		if (playa.PerkNamesArray[0]==1 && playa != None && Owner == None)
		{
								...
								if (VSize(dist) < proxRadius)
									if (skillTime == 0)
									{
									   ...
									   switch (itmp) //GMDX now pick what u want
									   {
									      ...
									      case 2:skillTime=2.5*difficultyMod;blastRadius=512.000000;break;
									      case 3:skillTime=7.0*difficultyMod;blastRadius=512.000000;break;
									   }
