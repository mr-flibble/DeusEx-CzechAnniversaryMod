	...
	else
	{
		bHasObject = (player.FindInventoryType(event.checkObject) != None);
		  if (bHasObject && player.FindInventoryType(event.checkObject).IsA('DeusExWeapon')) //CyberP: ignore the wep if it is modded
           ...
           if (wep.bHasScope || wep.bHasSilencer || wep.bHasLaser || wep.HasClipMod() || wep.HasDAMMod() || wep.HasROFMod() ||
...
//log("  invItemTo   = " $ invItemTo);
	...
	if (invItemTo != None)
	{
		...
		if (invItemTo.IsA('Ammo'))
		{
			...
			if (Ammo(invItemTo).IsA('Ammo10mm') && invokeActor.IsA('UNATCOTroop'))
                Ammo10mm(invItemTo).AmmoAmount = 11; //CyberP: Kaplan, stop giving the player shitloads of ammo
            else if (Ammo(invItemTo).IsA('Ammo762mm') && invokeActor.IsA('SamCarter'))
                Ammo762mm(invItemTo).AmmoAmount = 32; //CyberP: Sam Carter, you too.
...
//log("  invItemTo   = " $ invItemTo);
		...
		else
		{
			...
			if (invokeActor.IsA('PaulDenton'))    //CyberP: have paul offer you balanced choices
            {
             if (invItemTo.IsA('WeaponMiniCrossbow'))
             {
                conWinThird.ShowReceivedItem(invItemTo, itemsTransferred);
                invItemTo = Spawn(class'AmmoDartPoison');
              	invItemTo.GiveTo(Pawn(event.toActor));
              	conWinThird.ShowReceivedItem(invItemTo, itemsTransferred);
              	invItemTo = Spawn(class'AmmoDart');
              	AmmoDart(invItemTo).AmmoAmount = 6;
              	invItemTo.GiveTo(Pawn(event.toActor));
              	conWinThird.ShowReceivedItem(invItemTo, itemsTransferred);
              	invItemTo = Spawn(class'AmmoDartFlare');
              	AmmoDartFlare(invItemTo).AmmoAmount = 2;
              	invItemTo.GiveTo(Pawn(event.toActor));
             }
	...
	if ((DeusExPlayer(event.toActor) != None) && (conWinThird != None) && (invItemTo != None))
	{
		...
		player.PlaySound(sound'objpickup3',SLOT_None);
	}
	...
	else if ((invItemTo.IsA('DeusExPickup')) && (DeusExPickup(invItemTo).bCanHaveMultipleCopies))
	{
		...
		if (bSpawned)
		{
			...
			if (event.transferCount > 1)
			{
				...
				if (DeusExPickup(invItemTo).NumCopies > DeusExPickup(invItemTo).RetMaxCopies())
				{
					itemsTransferred = DeusExPickup(invItemTo).RetMaxCopies();
					DeusExPickup(invItemTo).NumCopies = DeusExPickup(invItemTo).RetMaxCopies();
				}
			}
		}
		...
		else
		{
			...
			if ((DeusExPickup(invItemTo).NumCopies + itemsTransferred) > DeusExPickup(invItemTo).RetMaxCopies()) //GMDX
				itemsTransferred = DeusExPickup(invItemTo).RetMaxCopies() - DeusExPickup(invItemTo).NumCopies; //GMDX
...
//   log("MYCHK: ConPlayBase.EEventAction : "$event.Name$" : points : "$event.pointsToAdd);
	event.pointsToAdd=0;
