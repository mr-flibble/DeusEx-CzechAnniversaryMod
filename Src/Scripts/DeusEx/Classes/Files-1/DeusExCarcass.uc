...
TakeDamage(200, Instigator, Location, vect(0,0,0), 'Shot');
}
// ----------------------------------------------------------------------
// Tick()
// ----------------------------------------------------------------------
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitLocation, Vector momentum, name damageType)
{
	    ...
	    (damageType == 'Tantalus') || (damageType=='throw') || (damageType == 'Burned'))  //Burned damage also
	{
		...
		if (player.bRealisticCarc || player.bHardCoreMode)  //CyberP: with this option enabled carcasses can only be damaged by explosions, plasma rifle and being eaten
        {
		  if ((damageType == 'Exploded') || (damageType == 'Munch') || (damageType == 'Burned' && Damage >= 10))
...
//log("DeusExCarcass::Frob()--------------------------------");
	...
	if (!bAnimalCarcass)
	{
	  ...
	  // one in inventory (like I did when giving the player starting inventory).
		...
		(player != None) && (player.inHand == None || (player.bAutoHolster && !player.inHand.IsA('POVCorpse'))) && (player.CarriedDecoration == None))
		{

			bDblClickStart=false;
			bDblClickFrob=false;
			if (player.inHand != None && player.bAutoHolster)
	  ...
	  // reasons it is a lot more of a hassle than it is worth.
		...
		if (Inventory != None)
		{
			...
			do
			{
				...
				if ((item.IsA('Ammo')) || (item.IsA('WeaponSpiderBotConstructor')) || (item.IsA('WeaponAssaultGunSpider')))   //CyberP: new type weapons exclusive to pawns
				{
					// Only let the player pick up ammo that's already in a weapon
					DeleteInventory(item);
					item.Destroy();
