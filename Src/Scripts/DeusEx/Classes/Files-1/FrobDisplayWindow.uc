		...
		if (frobTarget != None)
		{
			...
			if (frobTarget.IsA('Mover'))
			{
				...
				if ((dxMover != None) && dxMover.bLocked)
				{
					...
					gc.SetTileColor(col);
					gc.DrawPattern(infoX+(infoW-barLength-4), infoY+4+(infoH-8)/4, barLength*dxMover.lockStrength, ((infoH-8)/4)-2, 0, 0, Texture'ConWindowBackground');
					col = GetColorScaled(dxMover.doorStrength);
					gc.SetTileColor(col);
					gc.DrawPattern(infoX+(infoW-barLength-4), infoY+4+2*(infoH-8)/4, barLength*dxMover.doorStrength, ((infoH-8)/4)-2, 0, 0, Texture'ConWindowBackground');
			...
			else if (frobTarget.IsA('HackableDevices'))
			{
				...
				if (device.hackStrength != 0.0)
				{
					...
					gc.SetTileColor(col);
					gc.DrawPattern(infoX+(infoW-barLength-4), infoY+infoH/2.7, barLength*device.hackStrength, infoH/2.7-6, 0, 0, Texture'ConWindowBackground'); //CyberP: /2
				}
