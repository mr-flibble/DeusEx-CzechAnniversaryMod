	...
	if ((!bInvincible)&&(!IsA('ElectronicDevices'))&&(!IsA('Barrel1'))&&bHighlight&&!bExplosive&&bPushable)
	{
	  ...
	  switch fragType
	  {
		 ...
		 case class'WoodFragment' :
			...
			if (IsA('CouchLeather') || IsA('ChairLeather'))
		 ...
		 case class'MetalFragment' :
			...
			if (IsA('CrateUnbreakableLarge'))
             HitPoints=300;
			if (IsA('CrateUnbreakableMed') || IsA('WaterFountain') || IsA('HKBuddha'))
            { HitPoints=250; minDamageThreshold=40; }
			if (IsA('CrateUnbreakableSmall'))
...
function Landed(vector HitNormal)
{
	...
	if (Velocity.Z <= -150)
	{
		...
		AISendEvent('LoudNoise', EAITYPE_Audio,,512+(Mass*3)); //CyberP: mass factors into radius
	}
...
{
	if (!bInvincible && !IsA('CrateUnbreakableMed') && !IsA('CrateUnbreakableSmall'))
...
//			return;
    ...
    //   Other.TakeDamage(5000,Instigator,Location,Vect(0,0,0),'Exploded');
     if (Mass >= 40 && (Velocity.Z > 200 || Velocity.Z < -200) && (Other.IsA('ScriptedPawn') || Other.Mass < 40))
...
function Bump2( actor Other )
{
	...
	if( bPushable && (Pawn(Other)!=None) && (Other.Mass > 30) )
	{
		bBobbing = false;
		oldZ = Velocity.Z;
...
function Explode(vector HitLocation)
{
	...
	AISendEvent('LoudNoise', EAITYPE_Audio, , explosionRadius * 24); //CyberP: was 16
...
auto state Active
{
	function TakeDamage(int Damage, Pawn EventInstigator, vector HitLocation, vector Momentum, name DamageType)
	{
        ...
        Damage*= 0.5; //CyberP: less damage from baton, rubber bullets and master gas
    ...
    if (Mass < 100 && (bPushable || IsA('BookOpen') || IsA('BookClosed')))
    {
    dammult = damage*0.2;
    if (dammult < 0.8)
    dammult = 0.8;
    ...
    {
      if (EventInstigator.Weapon.IsA('WeaponSawedOffShotgun') || EventInstigator.Weapon.IsA('WeaponAssaultShotgun'))
      {
       damage*=5;      //CyberP: ensure shotguns send trivial objects flying and deal realistic damage.
       massMult+=0.3;  //CyberP: bPushable check above means objects are trivial (e.g no hackable devices)
       damMult=6;      //CyberP: relevant as we don't want to deal extra dam to turrets etc nor send them flying, of course!
      }
    }
...
simulated function Frag(class<fragment> FragType, vector Momentum, float DSize, int NumFrags)
{
	...
	{
	 for (i=0 ; i<NumFrags*20 ; i++)   //CyberP: plus 2
	{
	    vecta = Location;
