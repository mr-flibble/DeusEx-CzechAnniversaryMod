	...
	if (emitter != None)
	{
		...
		if ((emitter.HitActor != None) && (LastHitActor != emitter.HitActor))
		{
			if (IsRelevant(emitter.HitActor))
			{
                ...
                if (emitter.HitActor.IsA('MJ12Troop') || emitter.HitActor.IsA('Terrorist') || emitter.HitActor.IsA('MJ12Commando') || emitter.HitActor.IsA('MIB') || emitter.HitActor.IsA('WIB'))
                        {
                         bTrigger = False;  //CyberP: hacky. the baddies don't set off their own alarms.
                         }
...
function Trigger(Actor Other, Pawn Instigator)
{
	...
	if (emitter != None)
	{
		if (!bIsOn)
		{
			...
			Skin = Texture'HDTPlaserEmitterTex2';
			}
		}
	}
...
defaultproperties
{
     ...
     bDirectional=True
     DrawType=DT_Mesh
     Skin=Texture'HDTPDecos.Skins.HDTPlaseremittertex0'
