...
//      TrackingDeco.GotoState('');
	  Mass=TrackingDeco.Mass/1.1; //CyberP: divide by 3.5, was 4
	  mult += 2;
   } else
   {
...
//   mult*=FClamp(1+((Mass*Mass)/40000),1,2); //help heavier objects a touch

   //log("NEW MULT "@mult);
   //Speed =  mult * 250 + 40 * FRand();
   //MaxSpeed = (1+mult) * 250 + 40 * FRand();


   Velocity = Vector(DxPlayer.ViewRotation) * mult * 400;// + vect(0,0,220)+ 40 * VRand();
   Velocity.Z += 100;
   //log("Velocity1 "@Vsize(Velocity));
...
// scale it based on the mass
   ...
   //log("Initail Damage "@Damage);

   MomentumTransfer=Mass/2;


   //log("DECO DAMAGE:-"@Damage@" "@MomentumTransfer@" "@Speed@" "@VSize(Velocity));

	  // scale it based on the mass
//   velscale = FClamp(Mass / 20.0, 1.0, 40.0);
...
//   SetOwner(TrackingDeco);

   SetCollision(True,True,True);
	bCollideWorld = True;

	//if (Mass <= 50 && !TrackingDeco.IsA('Plant1') && !TrackingDeco.IsA('Plant2'))
	//		{
	//		TrackingDeco.bFixedRotationDir = True;
    //        TrackingDeco.RotationRate.Pitch = (32768 - Rand(65536)) * 6.0;
	//        TrackingDeco.RotationRate.Yaw = (32768 - Rand(65536)) * 6.0;
	//        }
	  ...
	  if(TrackingDeco.IsA('CrateUnbreakableSmall') || TrackingDeco.IsA('CrateUnbreakableMed')  || TrackingDeco.IsA('CrateUnbreakableLarge'))
	  {
		...
		else if (fragType == class'PlasticFragment')
		{
			if ((TrackingDeco.Mass <= 20)||FRand()<0.5)
				...
				CollideSound = sound'PlasticHit2';
		}
...
//log("DECO PROCESS TOUCH2 "@Other);
   ...
   function ProcessDamage (Actor Other, Vector HitLocation)
   {
      ...
      clampInfluence = DxPlayer.AugmentationSystem.GetAugLevelValue(class'AugMuscle');
      MassModifier = 1;
      ...
      MassModifier = 1;
      else if  (TrackingDeco.Mass < 100)
      MassModifier = 1.5;
      else if  (TrackingDeco.Mass < 400)
      MassModifier = 2;
      }
	  ...
	  if (Other!=None)
	  {
		...
		else if (Other.IsA('Pawn') && DxPlayer.AugmentationSystem != none && clampInfluence == 3) Dmg=250*MassModifier; //CyberP: bad coding. Meh, it will do.
        else if (Other.IsA('Pawn') && DxPlayer.AugmentationSystem != none && clampInfluence == 2) Dmg=FClamp(Dmg,1,Lerp(FClamp(Mass/250,0,1),70,200));
        else if (Other.IsA('Pawn') && DxPlayer.AugmentationSystem != none && clampInfluence < 2) Dmg=FClamp(Dmg,1,Lerp(FClamp(Mass/250,0,1),50,125));
	  ...
	  if (((TrackingDeco.IsA('DeusExCarcass')&&DeusExCarcass(TrackingDeco).CumulativeDamage<DeusExCarcass(TrackingDeco).MaxDamage))
		 ||!TrackingDeco.IsA('DeusExCarcass'))     //stop potential double gib carcass..
		 {
			//log("Tracking "@Dmg);
			if (TrackingDeco.Mass > 50)
			TrackingDeco.TakeDamage(Dmg,DxPlayer,HitLocation, 0.6*Velocity, 'throw'); //CyberP: let's just always destroy it
			else
			TrackingDeco.TakeDamage(Dmg+100,DxPlayer,HitLocation, 0.6*Velocity, 'throw'); //CyberP: let's just always destroy it
		 }
	  //if (Pawn(Other)==none)
	  //{
