...
singular function SupportActor(Actor standingActor)
{
    ...
    standingActor.TakeDamage(10000,None,standingActor.Location,vect(0,0,0),'Exploded'); //CyberP: terrible but safe hack to get pawns to gib to fan blades.
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, name damageType)
{
    ...
    if ((damageType == 'KnockedOut') && Damage > 90)
...
function Frob(Actor Frobber, Inventory frobWith)
{
	...
	if ((Player != None) && (msg != ""))
	{
	    if (msg == msgLocked && bBreakable && FragmentClass == Class'DeusEx.WoodFragment')
