...
function ChangePlayerVisibility(bool bInviso)
{
   ...
   if (Level.NetMode != NM_Standalone)
	...
	if (player.bRealUI || player.bHardCoreMode) //CyberP: Keep player visible when hacking.
	    return;
	if (player != None)
	...
	if (!bOn)
	{
      AdditionalActivation(curFrobber);
		...
		if (IsA('ComputerSecurity'))
            ...
            PlayAnim('Activate',1.1);
        if (IsA('ComputerPublic'))
            PlaySound(sound'publicterminalfrob',SLOT_None,0.5);
		FinishAnim();
		bOn = True;
		bAnimating = False;
...
defaultproperties
{
     ...
     alarmTimeout=30
     CompInUseMsg="The computer is booting up."
     Mass=20.000000
     Buoyancy=5.000000
}
