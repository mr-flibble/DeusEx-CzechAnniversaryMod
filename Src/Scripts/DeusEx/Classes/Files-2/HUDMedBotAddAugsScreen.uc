...
function InstallAugmentation()
{
    ...
    if (aug.IsA('AugHeartLung')) //CyberP: AugHeartLung upgrades all passive augs.
       ForEach Player.AllActors(class'Augmentation',allTheAugs)
        if (allTheAugs.bHasIt && allTheAugs.bAlwaysActive && allTheAugs.CurrentLevel != allTheAugs.MaxLevel)
          allTheAugs.CurrentLevel+=1;
...
defaultproperties
{
     ...
     clientTextures(3)=Texture'GMDXSFX.UserInterface.AugBackgroundMedDef'
