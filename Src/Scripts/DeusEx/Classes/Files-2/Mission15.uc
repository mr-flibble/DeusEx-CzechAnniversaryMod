...
function Timer()
{
	...
	else if (localURL == "15_AREA51_PAGE")
	{
		...
		if (FRand() < 0.05) //CyberP: we don't need this for loop happening every tick ffs
		{
		for (i=0; i<ArrayCount(spawnData); i++)
		{
			if ((spawnData[i].count == 0) && (Level.TimeSeconds - spawnData[i].lastKilledTime > 20))
			{
				...
				if (SP != None)
				{
					// draw some light effects   //CyberP: commented out for now
					//Spawn(class'ExplosionLight',,, SP.Location);
					Spawn(class'EllipseEffect',,, SP.Location);
					sphere = Spawn(class'SphereEffect',,, SP.Location);
					if (sphere != None)
					{
					    sphere.LightRadius = 0;
					    sphere.LightType = LT_None;
						sphere.size = 4.0;
                    }
					// draw some electricity effects   //CyberP: commented out for now
					/*for (j=0; j<4; j++)
					{
						loc = vect(0,0,0);
						loc.Z = 32.0 + 32.0 * j;
					...
					if (pawn != None)
					{
						...
						if (pawn.IsA('Gray'))
						    pawn.SetOrders('Attacking', spawnData[i].orderTag);
