...
function CreateRegionWindows()
{
   ...
   if (player.SkillSystem!=None)
   {
	...
	regionWindows[1] = CreateRegionWindow(1,  27,  43, player.HealthTorso,    player.default.HealthTorso+MedSkillAdd,    HealthLocationTorso);
	...
	else
	{
      ...
      if (player.SkillSystem!=None) //GMDX
      {
		...
		switch(region.GetPartIndex())
		{
			...
			case 1:		// torso
				newHealth = FMin(playerHealth[1] + pointsToHeal, player.default.HealthTorso+MedSkillAdd);   //GMDX
				healthAdded = newHealth - playerHealth[1];
				playerHealth[1] = newHealth;
				break;
