...
function AddAmmoInfoWindow(DeusExAmmo ammo, bool bShowDescriptions)
{
	...
	if (ammo != None)
	{
		...
		{
			winText.SetText(ammo.itemName @ "(" $ AmmoRoundsLabel @ ammo.AmmoAmount $ outOf $ ammo.MaxAmmo $ ")|n|n");
...
function AddSecondaryButton(DeusExWeapon wep)
{
   if (wep != None)
   {
    SetText("Assign as secondary weapon:");
	...
	buttonUpgrade.SetButtonText("Assign");
	assignThis = wep;
   AddLine();
   }
}
		...
		switch(buttonPressed)
		{
			case buttonUpgrade:
               ...
               if (assignThis != None && player.assignedWeapon != None && player.assignedWeapon == assignThis)
               {
                   ...
                   player.ClientMessage("Secondary Weapon Unassigned");
               }
               else if (assignThis != None)
               {
                   player.assignedWeapon = assignThis;
                   player.ClientMessage("Secondary Weapon Assigned");
               }
			   break;
