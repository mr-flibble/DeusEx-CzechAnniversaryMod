...
function Timer()
{
	...
	else if (localURL == "06_HONGKONG_WANCHAI_UNDERWORLD")
	{
			...
			!flags.GetBool('MS_CommandosUnhidden'))
		{
		 ...
		 //GMDX
			foreach AllActors(class'BreakableGlass',glassAT,'BreakableGlass')
			{
//			   log("FOUND BreakableGlass:-"@glassAT@" with event:-"@glassAT.Event);
            if (glassAT.Event=='AT')
            {
               glassAT.Event='';
//               log("BreakableGlass cleared Event");
            }
			}

			foreach AllActors(class'TriadRedArrow', TRA) //CyberP/|Totalitarian|: prevent Triads from getting mad during raid.
			{
			    TRA.bHateShot=False;
			    TRA.bHateDistress=False;
			}
	...
	Player.fullUp = 0; //CyberP: player can consume food again.
