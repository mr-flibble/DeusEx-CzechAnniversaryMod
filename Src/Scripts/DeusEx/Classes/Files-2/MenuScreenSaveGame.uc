...
// ----------------------------------------------------------------------
// GetLastSave()
// by dasraiser for GMDX: grab top most save game, should be newly created.
// ----------------------------------------------------------------------
// probably need to run this backward!
function int GetLastSave()
{
	local int saveIndex;
	local DeusExSaveInfo saveInfo;
	local GameDirectory saveDir;
   ...
   saveIndex=0;
   while ((saveInfo=saveDir.GetSaveInfoFromDirectoryIndex(saveIndex)==None)&&(saveIndex<saveDir.GetDirCount()))
   {
      saveIndex++;
   }
   if (saveIndex<saveDir.GetDirCount())
   {
      saveInfo.
      saveDir.DeleteSaveInfo(saveInfo);
      return saveIndex;
   }
   return -1;//handle some kind of error!
}
*/
