                              ...
                              int numClicks)
{
    ...
    if (button == IK_RightMouse && anItem.IsA('DeusExPickup') && (!anItem.IsA('Lockpick') || !anItem.IsA('Multitool')))
    {
       winInv.UseSelectedItem(); //winInv.ButtonActivated(??????, btnUse);
       bResult = True;
    }
    if (button == IK_RightMouse && (anItem.IsA('DeusExWeapon') || anItem.IsA('Lockpick') || anItem.IsA('Multitool')))
    {
