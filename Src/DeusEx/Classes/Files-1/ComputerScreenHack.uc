...
local string hackRequirement;
   ...
   if (Player.Level.NetMode == NM_Standalone)
    ...
    {               //CyberP: hack denied
      if (passedSecLevel == 2)
         hackRequirement = "Hacking Level ADVANCED Required";
      else if (passedSecLevel == 3)
         hackRequirement = "Hacking Level MASTER Required";
      else
         hackRequirement = "ERROR! REPORT THIS AS A BUG";
      Player.PlaySound(sound'cantdrophere',SLOT_None,2.0,,,0.8);
      winHackMessage.SetTextColor(colRed);
      SetHackMessage(hackRequirement);
      UpdateSoftwareNuke();
      return;
    }
}
