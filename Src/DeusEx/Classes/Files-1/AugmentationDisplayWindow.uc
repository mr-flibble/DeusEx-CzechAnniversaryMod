...
function Actor TraceLOS(float checkDist, out vector HitLocation)
{
	...
	target = None;
    //CyberP: upgrading the IFF aug results in xhairs always displayed
   /* if (Player.AugmentationSystem.GetAugLevelValue(class'AugIFF') >= 2.0)
    {
       ...
       foreach Player.TraceActors(class'Pawn', pawnie, HitLoc, HitNormal, EndTrace, StartTrace)
	   {
	     if (pawnie != None)
	     {
	     HitLocation = HitLoc;
	...
	{
		if ((target.IsA('Pawn') && !Pawn(target).IsInState('Dying')) || target.IsA('Flare') || target.IsA('WeaponLAM') || target.IsA('WeaponEMPGrenade')
         || target.IsA('DeusExDecoration') || target.IsA('ThrownProjectile') || target.IsA('WeaponGasGrenade') || target.IsA('WeaponNanoVirusGrenade') ||
			(target.IsA('DeusExMover') && DeusExMover(target).bBreakable) ||
            (target.IsA('DeusExPickup') && DeusExPickup(target).bBreakable))
		{
...
local Actor acti;
 ...
 if (acti == none)
 {
  ...
  if (threatType != "" && threatType != "shot" && threatType != "fell")
  {
    if (bHazardRefresh)
...
function GetTargetReticleColor( Actor target, out Color xcolor )
{
    ...
    if ( target.IsA('DeusExPickup'))  //CyberP: pickups display xhair
    xcolor = colWhite;
...
function DrawTargetAugmentation(GC gc)
{
	...
	if ( target != None )
	{
		...
		if ((weapon != None) && !Player.IsInState('Dying') && (!weapon.bHandToHand || weapon.IsA('WeaponShuriken')) && !bUseOldTarget && !(weapon.IsA('WeaponGEPGun')&& WeaponGEPGun(weapon).GEPinout>=1.0)) //GMDX:remove IFF from overlaying GEP
		{
