...
function BeginPlay()
{
	...
	if (minDamageThreshold == 80)
    {hackStrength=0.600000; HitPoints = 150;}

	if (bSwing == False)
	{
	bSwing = True;  //CyberP: bad lazy hack so non-swinging cameras reset to default rot after tagging player/corpse.
	swingAngle = 2;
    }
	SoundRadius=96;
	...
	if (player.bDetectable && !player.bIgnore && (dist <= cameraRange))
	{
		...
		if (hit == player)
		{
			...
			if ((Abs(yaw) < cameraFOV) && (Abs(pitch) < cameraFOV))
			{
                ...
                if (minDamageThreshold == 80 && bTrigSound == False)
		       {
               bTrigSound = True;
...
function Tick(float deltaTime)
{
    ...
    if (!bPlayerSeen && !bCarcassSeen && minDamageThreshold == 80)
    {
        if (bTrigSound)
    ...
    if (curplayer != None)
    {
    skillz = curplayer.SkillSystem.GetSkillLevel(class'SkillStealth');
    if (curplayer.SkillSystem != none && skillz >= 2)
    triggerDelay = 4.200000;
    }
...
auto state Active
{
	function TakeDamage(int Damage, Pawn EventInstigator, vector HitLocation, vector Momentum, name DamageType)
	{
        ...
        PlaySound(sound'ArmorRicochet',SLOT_None,,,1280);
