	...
	if (CurrentLevel < 3)
	{
		...
		if (localPlayer != None)
		{
			if ((localPlayer.SkillPointsAvail >= Cost[CurrentLevel]))
			{
				...
				localPlayer.HealthTorso=Min(localPlayer.HealthTorso+CurrentLevel*10.0,100.0+CurrentLevel*10.0);
...
defaultproperties
{
     ...
     PerksDescription3="|nAn agent can carry five additional medkits and biocells each. |n|nRequires: Medicine: MASTER"
     PerkName="BIOGENIC"
