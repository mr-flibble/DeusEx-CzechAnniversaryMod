...
function ShouldGreaselMelee() //CyberP: somehow gets called out of the desired state and crashes, so this prevents that.
   {
   }
...
function bool HasEnemyTimedOut()
{
	if (EnemyTimeout > 0)
	{
	    if (EnemyLastSeen > EnemyTimeOut && bDefendHome && HomeExtent > 64)
	    {bDefendHome = false; EnemyLastSeen *= 0.3;}  //CyberP: yet another hack
...
function Carcass SpawnCarcass()
{
	...
	if ((Health < -120) && !IsA('Robot')) //CyberP: was -100
	{
		size = (CollisionRadius + CollisionHeight) / 2; //CyberP: was 2
		...
		if (size > 10.0)
		{
			...
			for (i=0; i<size/1.1; i++) //CyberP: was /1.2
			{
				...
				if (chunk != None)
				{
				   if (bBurnedUp)
					{
					chunk.Velocity.Z = FRand() * 280 + 280;
					...
					{
                    chunk.Velocity.Z = FRand() * 370 + 370;
                 	chunk.DrawScale = size / 22; //Cyberp: was 22
					...
					chunk.RotationRate = RotRand()*1000; //CyberP: was False
					}
       			}
			}
		}
							...
							Vector offset, Name damageType)
{
	...
	if (bStunned)
		actualDamage *= 8; //CyberP: override bug: now *8, hacky fix
...
function EHitLocation HandleDamage(int actualDamage, Vector hitLocation, Vector offset, name damageType, optional Pawn instigatedBy)
{
	...
	if (actualDamage > 0)
	{
		...
		{
		    if (offset.z > 41.0 && bHasHelmet && damageType == 'Shot') //offset.z > 41.0
                {
                    PlaySound(sound'ArmorRicochet',SLOT_None,,,1536);
                    if (!IsA('MJ12Troop') && !IsA('MJ12Elite') && !IsA('UNATCOTroop') && (actualDamage >= 25 || FRand() < 0.04))
                    {
                          HelmetBreak();
                    }
                    else if (IsA('UNATCOTroop') && (actualDamage >= 25 || FRand() < 0.07))
                    {
                          HelmetSpawn(hitLocation, actualDamage, instigatedBy);
                          actualDamage = 0;
                          damageType = '';
                    }
                    else
			...
			else  // sides of head treated as torso  //CyberP: hmm
			{
			    if ((damageType == 'Stunned') || (damageType == 'KnockedOut'))
					HealthTorso -= actualDamage * 5;
				else
				    HealthTorso -= actualDamage * (8+extraMult);     //CyberP: lets just multiply the damage as head.
				if (offset.x < 0.0)
				{
		...
		else						// arms and torso
		{
			...
			{
				HealthTorso -= actualDamage * 3; //CyberP: was *2. Make torso a bit more vulnerable
				if (offset.x < 0.0)
				{
					hitPos = HITLOC_TorsoBack;
					bFlyer=False;
						...
						bool bPlayAnim)
{
    ...
    if (instigatedBy != None && instigatedBy.IsA('DeusExPlayer'))
    {
     if (DeusExPlayer(instigatedBy).bHitmarkerOn)
     {
         ...
         //DeusExPlayer(instigatedBy).PlaySound(sound'GMDXSFX.Weapons.HitmarkerSFX',SLOT_Interface,,,256);
	 }
     if (DeusExPlayer(instigatedBy).inHand != None)
     {
     if (DeusExPlayer(instigatedBy).inHand.IsA('WeaponNanoSword') || DeusExPlayer(instigatedBy).inHand.IsA('WeaponCrowbar'))
     extraMult = -2;
     else if ((DeusExPlayer(instigatedBy).inHand.IsA('DeusExWeapon') && DeusExWeapon(DeusExPlayer(instigatedBy).inHand).ItemName == "USP.10") ||
     DeusExPlayer(instigatedBy).inHand.IsA('WeaponSawedOffShotgun'))
     extraMult = 1;
     }
    }
    ...
    HitSound1 = none;  //CyberP: no sound for poisoned augged MiBs

    if (damageType != 'Poison' && actualDamage > 0 && GroundSpeed>default.GroundSpeed && Fatness != 130)
    ...
    {
    player = DeusExPlayer(GetPlayerPawn());
    if (DamageType == 'Shot' && (Damage >= 25 || player.inHand.IsA('WeaponAssaultShotgun') ||
     player.inHand.IsA('WeaponSawedOffShotgun'))) //CyberP: you need to change these conditions
    {
    ...
    if (FRand() < 0.3)
       ...
       player = DeusExPlayer(GetPlayerPawn());
        if (bCanPop && player.bHDTP_ALL == -1 && player.bDecap && (Damage >= 25 || player.inHand.IsA('WeaponAssaultShotgun') ||
     player.inHand.IsA('WeaponSawedOffShotgun'))) //CyberP: you need to change these conditions
        {
        bSetupPop = True;
...
function ResetSkinStyle()
{
	...
	Style     = Default.Style;
	if (IsA('MJ12Troop') && UnfamiliarName == "MJ12 Elite")
	{
...
function PlayDyingSound()
{
	...
	AISendEvent('LoudNoise', EAITYPE_Audio,0.25,490);
	if (bEmitDistress)
		AISendEvent('Distress', EAITYPE_Audio,0.25,224); //CyberP: radius was 490
}
...
function PlayAreaSecureSound()
{
	...
	dxPlayer = DeusExPlayer(GetPlayerPawn());
	if ((dxPlayer != None) && (Enemy == dxPlayer) && (FRand() < 0.7)) //CyberP: cutting down on pawns barking same lines
...
function PlayTakeHitSound(int Damage, name damageType, int Mult)
{
	...
	if ((hitSound != None) && bEmitDistress)
		AISendEvent('Distress', EAITYPE_Audio, volume*0.75); //CyberP: *0.75 added
}
...
function PlayFootStep()
{
	...
	if (WalkSound == None)
	{
		...
		else
		{
            ...
            dxPlayer = DeusExPlayer(GetPlayerPawn());
            if (dxPlayer.bThisMission && dxPlayer != none)
			{
			if (rnd < 0.25)
...
//	volume      = (speedFactor+0.7)*massFactor;
	...
	radius      = 1536.0;
		...
		if (dxPlayer != None)
		{
			...
			shakeRadius = FClamp((Mass-400)/300, 0, 1.3) * (range*0.6);  //CyberP: modded all this to make milbots more scary
			shakeMagnitude = FClamp((Mass-400)/1600, 0, 1.3);
			shakeMagnitude = FClamp(1.0-(playerDist/shakeRadius), 0, 1.3) * shakeMagnitude;
			if (shakeMagnitude > 0)
...
function PlayTakingHit(EHitLocation hitPos)
{
	...
	if (animName != '')
		PlayAnimPivot(animName,0.9 , 0.1, pivot); //CyberP: 0.9 added
		...
		if (alarmInstigator != None)
		{
			if (alarmInstigator.Health > 0)
			{
				if (IsValidEnemy(alarmInstigator))
				{
					AlarmTimer = 120;
					SetDistressTimer();
					if (IsA('HumanMilitary') && !bAlarmStatIncrease)  //CyberP: AI get better stats if alarm is triggered.
					{
                    ...
                    if (FRand() < 0.5)
					    SetSeekLocation(alarmInstigator, alarmLocation, SEEKTYPE_Guess, true);  //CyberP: different reaction
					else
							...
							optional vector extent, optional bool bIgnoreLevel)
{
	                    ...
	                    traceEnd, traceStart, extent)
	{
		...
		if (traceActor == Level)
		{
		    //dista = Abs(VSize(Enemy.Location - traceActor.Location)); //CyberP: supressive fire
		    //if (dista < 512)
            //    break;
			if (!bIgnoreLevel)
				bSafe = false;
		...
		if (traceDecoration != None)
		{
			...
			//if ((traceDecoration.HitPoints > 100) || (traceDecoration.minDamageThreshold > 20))  // hack //CyberP: hp was 20, dt was 4 //CyberP: actually, comment out
			//{
			//	bSafe = false;
			//	break;
			//}
		...
		if (traceMover != None)
		{
			...
			else if ((traceMover.minDamageThreshold > 3))  // hack  //CyberP: get them to shoot through breakable glass
			{
				bSafe = false;
				break;
			}
	...
	else
	{
		...
		if (bIsThrown && (throwAccuracy > 0))
			return (AISafeToThrow(projEnd, projStart, throwAccuracy,
			                      extent));
		else if (dxWeapon.IsA('WeaponPlasmaRifle'))         //CyberP: so plasma rifle users can attack close range
        	return (AISafeToShoot(hitActor, projEnd, projStart, extent*1.1));
		else
...
{
	if (isA('MIB')||isA('WIB')) return false;
...
//	if ( Target == None )
	...
	if (leadTarget && !IsA('Gray') && FRand() < 0.7 && (projSpeed > 0))  //CyberP: grays never target lead
	{
		if (bIsThrown)
		{
			// compute target's position 1.5 seconds in the future
...
function Tick(float deltaTime)
{
	...
	if (bTickVisibleOnly)
	{
	 	if (DistanceFromPlayer > 1200)
		  ...
		  if (player !=None && player.CombatDifficulty > 2) //CyberP: realistic & hardcore have realistic vision
				ModDistance=4096.0;
					 else ModDistance=2590.0;
...
function ZoneChange(ZoneInfo newZone)
{
	...
	if (newZone.bWaterZone)
	{
		...
		if (IsA('Doberman'))     //CyberP: Doberman have no swim animations, so kill them.
		    TakeDamage(100,self,vect(0,0,0),vect(0,0,0),'Shot');
		if (!bCanSwim)
			MoveTimer = -1.0;
...
function CheckDestLoc(vector newDestLoc, optional bool bPathnode)
{
	...
	if (bEnableCheckDest && (DestAttempts >= 4))   //CyberP: hmm...
		BackOff();
}
...
//GMDX: HandleGenericNoise test
	...
	function BeginState()
	{
		...
		SetupWeapon(true);
		SetDistress(false);
        bInterruptState=True; //CyberP: commented out for now //set back
		EnableCheckDestLoc(false);
	}
...
TurnToLocation:
	...
	PlayWaiting();
	if (SeekType == SEEKTYPE_Sound && SeekLevel == 0)
	{
	Sleep(FRand()*2+3);
	...
	/*if (disturbanceCount > 16 && disturbanceCount < 19)
    {
        ...
        if (SeekType == SEEKTYPE_Sight)     //CyberP: if type sight, sleep for FRand()+1.
        {
            ...
            Sleep(FRand()+1.0);
            if (FRand() < 0.4)    //CyberP: begin
            {
            SeekType = SEEKTYPE_Guess;
            GoTo('GoToLocation');
            }        //CyberP: end. This makes pawns sometimes seek the location
            else
            {
              if ((IsA('Terrorist') || IsA('MJ12Troop') || IsA('UNATCOTroop')) && !bHasThrownFlare) //CyberP: flare throw
                checkFlareThrow();
              if (bCanFlare)
              {
                FinishAnim();
	...
	Sleep(FRand()+0.2); //CyberP: else we don't want much sleeping on the job. :)
...
DoneSeek:
	if (bSeekPostCombat)
		PlayTargetLostSound();
	else if (RaiseAlarm != RAISEALARM_BeforeAttacking) //CyberP: bad hack so they don't say anything after alarm trigger
		PlaySearchGiveUpSound();
...
State Attacking
{
	...
	function EDestinationType PickDestination()
	{
		...
		if (destType == DEST_Failure)
		{
			...
			//if (IsHandToHand())
			//{destType = DEST_NewLocation; destLoc = Location+(VRand()*1200);} //CyberP: run away if cannot reach player with melee
	...
	function bool FireIfClearShot()
	{
		...
		if (dxWeapon != None)
		{
			...
			if ((dxWeapon.AIFireDelay > 0) && (FireTimer > 0))
				return false;
			else if (dxWeapon.IsA('WeaponRifle') && FRand() < 0.8 && Enemy.IsA('DeusExPlayer') && DeusExPlayer(Enemy).CombatDifficulty < 3)
            	    return false;  //CyberP: rand as to whether snipers can fire, to simulate taking time to aim.
	...
	function StartCrouch()
	{
		if (!bCrouching)
		{
			...
			CrouchTimer = 1.0+FRand()*0.75; //CyberP: was *0.5
		}
	}
   ...
   dxPlayer = DeusExPlayer(GetPlayerPawn());
   dista = Abs(VSize(dxPlayer.Location - Location));
   //CyberP: I should probably change this to apply to enemy rather than just dxPlayer
   if (dista < 128)
   {
   StartTrace = Location;
   EndTrace = Location + (Vector(Rotation)) * (CollisionRadius+40);

   if (dxPlayer != None && Trace(HitLocation, HitNormal, EndTrace, StartTrace, True) == dxPlayer)
   {
   ...
   function ShouldRadialBlast()
   {
	...
	if (Enemy!=none)
        {
   		dist = Abs(VSize(Enemy.Location - Location));  //CyberP: should change this to trace so they cannot do so through walls.
    ...
    if (playa != none && bCanNade == False)
    {
     //CyberP: trace towards player. If we manage to trace end near the player AND there is no other pawns close to that location, then we can throw.
    dista = Abs(VSize(playa.Location - Location));
    if (dista > 768 && dista < 1536 && (Abs(Location.Z-playa.Location.Z) < 160))
    {
     offset = Location;
     offset.Z += 8;
     StartTrace = offset;
     EndTrace = playa.Location;
        ...
        ForEach Trace(HitLocation, HitNormal, EndTrace, StartTrace, True).RadiusActors(Class'DeusExPlayer',playa,320,HitLocation)
        {
          i++;
          if (i == 1)
          {
              i = 0;
              ForEach playa.RadiusActors(Class'HumanMilitary',P,640,playa.Location)
              {
                 i++;
              }
             if (i < 1)
    ...
    function nadeThrow() //CyberP: the throwing itself is exceptionally bad, but hey, it is still better than the vanilla nade throwing.
    {
    ...
    if (IsA('Terrorist') || IsA('UNATCOTroop') || IsA('RiotCop'))
    {
     gasNade = Spawn(class'GasGrenade',self,,flareOffset);
     if (gasNade != None)
     {
     if (playa != None && playa.CombatDifficulty > 2)
          gasNade.blastRadius += 256;
       if (dista < 1000)
       {
       gasNade.Velocity = Vector(Rotation)*1000 + (VRand()*6);
       gasNade.Velocity.Z += 300;
       }
       else if (dista < 1300)
       {
       gasNade.Velocity = Vector(Rotation)*1150 + (VRand()*6);
       gasNade.Velocity.Z += 350;
       }
       else
       {
       gasNade.Velocity = Vector(Rotation)*1300 + (VRand()*6);
       gasNade.Velocity.Z += 400;
       }
     }
    }
    else if (IsA('MJ12Troop') || IsA('Soldier') || IsA('MJ12Elite'))
    {
      ...
      {
        if (playa != None && playa.CombatDifficulty > 2)
            empNade.blastRadius += 256;
        if (dista < 900)
       {
       empNade.Velocity = Vector(Rotation)*1000 + (VRand()*6);
       empNade.Velocity.Z += 300;
       }
       else if (dista < 1300)
       {
       empNade.Velocity = Vector(Rotation)*1150 + (VRand()*6);
       empNade.Velocity.Z += 350;
       }
       else
       {
       empNade.Velocity = Vector(Rotation)*1300 + (VRand()*6);
       empNade.Velocity.Z += 400;
       }
      }
      }
    }
	...
	while (PickDestination() == DEST_NewLocation)
	{
		...
		else
		{
	        ...
	        if (bGreaselShould)
            {
                   ...
                   RotationRate.Yaw = 2;
                   switchToBestWeapon();
                   PlayAnimPivot('Attack',2.6,0.1);
                   SetPhysics(PHYS_Falling);
                   Velocity = (Vector(Rotation)) * 400;
                   Velocity.Z = 260 + (FRand()*20);
                   bGreaselShould=False;
                   Sleep(0.3);
                   FinishAnim();
                   LoopAnimPivot('Run',runAnimMult,0.1);
...
Fire:
	...
	if (IsA('HumanMilitary'))
	{
       ...
       if (IsA('MJ12Commando'))
          Enemy.Velocity -= Vector(Enemy.ViewRotation) * 900;
       else
          Enemy.Velocity -= Vector(Enemy.ViewRotation) * 350;
       Enemy.Velocity.Z = 20;
	...
	{
              if ((IsA('MJ12Troop') || IsA('MJ12Elite')) && FRand() < 0.03)
                    checkNadeThrow();
              else if (FRand() < 0.02)
                    checkNadeThrow();
              if (bCanNade)
...
ContinueFire:
    ...
    if (IsA('HumanMilitary'))
	{
       ...
       if (IsA('MJ12Commando'))
          ...
          Enemy.Velocity -= Vector(Enemy.ViewRotation) * 400;
       Enemy.Velocity.Z = 20;
       Enemy.PlaySound(sound'pl_fallpain3',SLOT_None,2.0,,,1.2);
	   sleep(0.05);
	   FinishAnim();
...
Begin:
	if (Enemy == None && RaiseAlarm != RAISEALARM_BeforeAttacking) //CyberP: added extra condition for pawns who spot carci
		GotoState('Seeking');
	//EnemyLastSeen = 0;
	destPoint = None;
	if (RaiseAlarm == RAISEALARM_Never)
...
state OpeningDoor
{
	...
	local DeusExMover moved;
	    ...
	    if (moved != none && moved.MoveTime <= 1.5)
	    ResetReactions();     //CyberP: this resets reactions early for fast-moving doors
...
BeginHitNormal:
	...
	if (DoorEncroaches())
		if (!FrobDoor(Target))
			Goto('DoorOpened');
	PlayWaiting();
	if (target != None && target.IsA('DeusExMover') && DeusExMover(target).MoveTime < 5.0 && DeusExMover(target).bIsDoor) //CyberP: added this block, because Sleeping for 5 secs in combat is bad.
	{
       if (enemy != None && DeusExMover(target).MoverEncroachType == ME_IgnoreWhenEncroach)
       {
          Sleep(DeusExMover(target).MoveTime*0.4);
	      FollowOrders();
	   }
	   else
          Sleep(DeusExMover(target).MoveTime);
    }
	else
	   Sleep(5.0);
...
state TakingHit
{
	...
	local DeusExPlayer player;
        ...
        if (player.PerkNamesArray[14]==1 && player.inHand != None && DeusExWeapon(player.InHand).bHandToHand)
		TakeHitTimer = 1.3;
		else if (IsA('Gray'))   //CyberP: grays are formidable
		TakeHitTimer = 10.0;
		else
